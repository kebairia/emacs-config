...
#+STARTUP: overview
#+SETUPFILE: ~/org/files/conf/readtheorg/readtheorg.conf
...
*** Garbage Collection
    ...
    #+end_src
*** COMMENT Fixing the COPY/CUT/PASTE Behaviour
    - =shift-insert= to paste that text you highlighted with the mouse
    - If you are having issues with cutting and pasting between emacs and other applications,
      - it may be an issue with interactions between the UI’s clipboard or selection buffer and the emacs.
      - Windows has a single clipboard buffer. X11 has more than one buffer. Consider these solutions:
    - check [[https://www.emacswiki.org/emacs/Comments_on_CopyAndPaste][this]] page for more explanation:
    #+begin_src emacs-lisp
      ;- Clipboard stuff ---------------------
...
** TODO Documentation
   ...
   :END:
   |-------------------------------------+-------------+----------------------------|
   | Config                              | Description | Link                       |
   |-------------------------------------+-------------+----------------------------|
   | =uniquify-buffer-name-style 'forward= |             | [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Uniquify.html][Making Buffer Names Unique]] |
   |                                     |             |                            |
** TODO breakup your =init.el= into files
  :PROPERTIES:
  :header-args: :tangle no
  :END:
  #+begin_src elisp
